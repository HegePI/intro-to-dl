...
class Model(torch.nn.Module):
    def __init__(self,  vocab_size: int, embedding_dim: int, lstm_hidden_dim: int, num_classes: int):
    ...
    def forward(self, seqs: torch.Tensor, seqs_lengths: torch.Tensor):
        ...
        max_pool = torch.nn.functional.F.adaptive_max_pool1d(
            ouput.permute(1, 2, 0), 1
        ).view(seqs.size(1), -1)

        linear_output = self.linear(torch.cat([max_pool], dim=1))

        return torch.nn.functional.F.log_softmax(linear_output, dim=1)
